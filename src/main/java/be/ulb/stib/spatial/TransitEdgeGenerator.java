...
public final class TransitEdgeGenerator {
    ...
    public static void build(GlobalModel model) {
        ...
        ensureSize(model.transitEdgesRouteIdx,  nStops, null);
        ...
        for (int j = 0; j < ofsDense.size() - 1; j++) {
            ...
            int tripIdx = model.tripOfsSparse.indexOf(from);
            ...
            for (int i = from; i < to - 1; i++) {
                ...
                // stopA → stopB
                addEdge(model, stopA, stopB, cost, tripIdx);
            }
        }
    }
     ...
     */
    private static void addEdge(GlobalModel model, int stopIdxA, int stopIdxB, int cost, int tripIdxA) {
